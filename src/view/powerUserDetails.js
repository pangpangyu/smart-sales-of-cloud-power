...
export default class Todolist extends React.Component {
	...
	//处理年度预计电量的数据
	handleYearPowerData = (obj) => {
		let yearPowerTab = []
		yearPowerTab[0] = obj.jan
		yearPowerTab[1] = obj.feb
		yearPowerTab[2] = obj.mar
		yearPowerTab[3] = obj.apr
		yearPowerTab[4] = obj.may
		yearPowerTab[5] = obj.jun
		yearPowerTab[6] = obj.jul
		yearPowerTab[7] = obj.aug
		yearPowerTab[8] = obj.sep
		yearPowerTab[9] = obj.oct
		yearPowerTab[10] = obj.nov
		yearPowerTab[11] = obj.dec_
		return yearPowerTab
	}
	//年度预计电量
	getYearPowerTableData = () => {
		...
		api.GetPowerYearEstimate(params).then(res => {
			if(res){
				if (res.status === 0) {
					let yearPowerTab = this.handleYearPowerData(res.data.rows[0])
					let YearPowerPickerView = []
					res.data.rows.map(item => {
						YearPowerPickerView.push({label:item.yearDate,value:item.yearDate})
					})
					this.setState({
						yearPowerTab: yearPowerTab,
						electricTotal: res.data.rows[0].total,
						year: res.data.rows[0].yearDate,
						yearPowerData:res.data.rows,
						YearPowerPickerView:YearPowerPickerView
					})
				}
			}else{
				this.setState({
	...
	YearPowerTable = () => {
		return (
			<div className="annualEstimatedPower">
				...
				{!this.state.yearPowerTabNull && <div>
				...
				{this.state.yearPowerTabNull && <NoData/>}
				...
				<div className={this.state.open ? 'modal on' : 'modal'}>
					...
					<div className="pick_box">
						{/* <DatePickerView
							minDate={this.state.YearPowerPickerView[0]}
							maxDate={this.state.YearPowerPickerView[this.state.YearPowerPickerView.length-1]}
							mode="year"
							locale={enUs}
						...
						/> */}
						<PickerView
							data={this.state.YearPowerPickerView}
							onChange={this.onChange}
							cascade={false}
							value={this.state.time}
						/>
